...
def reformat_space_html(space_key: str) -> dict:
    """Reformat all HTML files in a space by re-applying prettify_html.

    This is useful when the prettification logic has been updated (e.g., spacing fixes)
    and you want to apply it to existing local files without re-syncing from Confluence.

    Args:
        space_key: The space key to reformat

    Returns:
        Dict with reformatted count and any errors
    """
    space_path = get_space_path(space_key)
    if not space_path.exists():
        return {"error": f"Space '{space_key}' not found locally"}

    reformatted = 0
    errors: list[str] = []

    # Find all HTML files in the space (excluding metadata)
    for html_file in space_path.rglob("*.html"):
        try:
            content = html_file.read_text(encoding="utf-8")

            # Split header comment from body
            # Header format: <!--\n  ...\n-->\n
            ...
            header = content[: header_end + 4]  # Include "-->\n"
            body = content[header_end + 4 :]

            # Re-prettify the body (this applies fix_html_spacing)
            prettified_body = prettify_html(body)

            # Write back
            html_file.write_text(header + prettified_body, encoding="utf-8")
            reformatted += 1
            logger.debug(f"Reformatted: {html_file.name}")

        except Exception as e:
            errors.append(f"{html_file.name}: {str(e)}")
            logger.error(f"Error reformatting {html_file}: {e}")
